    ...
    _block_ids()
{
  if (_aux_disp.size() != _coupled_disp.size())
    mooseError("Number of auxiliary displacements variables should be equal to the number of "
               "displacement variables, i.e., ",
               _coupled_disp.size());
}
...
GlobalStrainAction::act()
{
  ...
  else if (_current_task == "add_scalar_kernel")
  {
    ...
    params.applyParameters(parameters(), {"global_strain_uo"});
  ...
  else if (_current_task == "add_material")
  {
    ...
    params.applyParameters(parameters(),
                           {"use_displaced_mesh", "scalar_global_strain", "global_strain_uo"});
  ...
  else if (_current_task == "add_aux_kernel")
  {
    for (unsigned int i = 0; i < _aux_disp.size(); ++i)
    {
      ...
      params.applyParameters(parameters(), {"scalar_global_strain", "global_strain_uo"});
