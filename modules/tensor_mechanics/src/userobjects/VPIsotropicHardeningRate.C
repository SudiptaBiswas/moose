...
VPIsotropicHardeningRate::computeTensorDerivative(unsigned int qp, const std::string & coupled_var_name, RankTwoTensor & val) const
{
  ...
  {
    RankTwoTensor pk2_dev = computeDeviatoricStress(_pk2[qp], _ce[qp]);
    Real eqv_stress = computeEqvStress(_pk2[qp], _ce[qp], _intvar_tensor[qp]);
    Real dhardrate_deqvstress = _exponent * std::pow(macaulayBracket(eqv_stress / _strength[qp] - 1.0), _exponent - 1.0) / _strength[qp];
    RankTwoTensor back_stress = 2/3 * _C * _intvar_tensor[qp];
    RankTwoTensor sdiff = _pk2[qp] - back_stress;
...
Real
VPIsotropicHardeningRate::computeEqvStress(const RankTwoTensor & pk2, const RankTwoTensor & ce, const RankTwoTensor & intvar) const
{
...
RankTwoTensor
VPIsotropicHardeningRate::computeDeviatoricStress(const RankTwoTensor & pk2, const RankTwoTensor & ce) const
{
