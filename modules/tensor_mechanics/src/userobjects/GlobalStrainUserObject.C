...
GlobalStrainUserObject::execute()
{
  ...
  for (unsigned int _qp = 0; _qp < _qrule->n_points(); _qp++)
  {
    ...
    eigenstress_tensor(0, 0) = _C11 * Utility::pow<2>(_polar_x[_qp]) * _Q11 +
    ...
    eigenstress_tensor(1, 1) = _C12 * Utility::pow<2>(_polar_x[_qp]) * _Q11 +
    ...
    eigenstress_tensor(2, 2) = _C12 * Utility::pow<2>(_polar_x[_qp]) * _Q11 +
                               _C12 * Utility::pow<2>(_polar_y[_qp]) * _Q11 +
    ...
    _residual +=
        _JxW[_qp] * _coord[_qp] * (_stress[_qp] - _applied_stress_tensor - eigenstress_tensor);
