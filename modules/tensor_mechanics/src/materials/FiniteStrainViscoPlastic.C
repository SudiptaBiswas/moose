...
InputParameters validParams<FiniteStrainViscoPlastic>()
{
  ...
  params.addClassDescription("Material class for rate dependent visco-platic flow: Can handle multiple flow models defined by flowratemodel type user objects");
  return params;
}
...
void
FiniteStrainViscoPlastic::initNumUserObjects(const std::vector<UserObjectName> & uo_names, unsigned int & uo_num)
{
...
template <typename T>
void
FiniteStrainViscoPlastic::initProp(const std::vector<UserObjectName> & uo_names, unsigned int uo_num, std::vector<MaterialProperty<T> *> & uo_prop)
{
...
template <typename T>
void
FiniteStrainViscoPlastic::initPropOld(const std::vector<UserObjectName> & uo_names, unsigned int uo_num, std::vector<MaterialProperty<T> *> & uo_prop_old)
{
...
template <typename T>
void
FiniteStrainViscoPlastic::initUserObjects(const std::vector<UserObjectName> & uo_names, unsigned int uo_num, std::vector<const T *> & uo)
{
  uo.resize(uo_num);
...
void
FiniteStrainViscoPlastic::computeQpStress()
{
  bool converge;
...
bool
FiniteStrainViscoPlastic::solveQp()
{
  preSolveFlowrate();
  if (!solveFlowrate())
    return false;
...
void
FiniteStrainViscoPlastic::postSolveQp()
{
  recoverOldState();
...
bool
FiniteStrainViscoPlastic::solveFlowrate()
{
  Real resid0, rnorm;
  unsigned int iter = 0;
...
void
FiniteStrainViscoPlastic::computeFlowRateJacobian()
{
...
bool
FiniteStrainViscoPlastic::computeFlowDirection()
{
...
bool
FiniteStrainViscoPlastic::computeFlowRateFunction()
{
  Real val = 0;
  for (unsigned i = 0; i < _num_flow_rate_uos; ++i)
  {
...
void
FiniteStrainViscoPlastic::computeElasticStrain()
{
  RankTwoTensor iden;
  iden.addIa(1.0);
...
void
FiniteStrainViscoPlastic::computeDeeDce()
{
  _dee_dce.zero();
...
void
FiniteStrainViscoPlastic::computeElasticRightCauchyGreenTensor()
{
  _ce[_qp] = _fe.transpose() * _fe;
}

void
FiniteStrainViscoPlastic::computeElasticPlasticDeformGrad()
{
  RankTwoTensor iden;
  iden.addIa(1.0);
...
void
FiniteStrainViscoPlastic::computeDpk2Dfpinv()
{
  for (unsigned int i = 0; i < LIBMESH_DIM; ++i)
    for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
      for (unsigned int k = 0; k < LIBMESH_DIM; ++k)
...
Real
FiniteStrainViscoPlastic::computeNorm(const std::vector<Real> & var)
{
  Real val = 0.0;
...
void
FiniteStrainViscoPlastic::updateFlowRate()
{
...
void
FiniteStrainViscoPlastic::computeQpJacobian()
{
...
bool
FiniteStrainViscoPlastic::computeIntVarRates()
{
  Real val = 0;
  for (unsigned int i = 0; i < _num_int_var_rate_uos; ++i)
  {
...
bool
FiniteStrainViscoPlastic::computeStrength()
{
  Real val = 0;
  for (unsigned int i = 0; i < _num_strength_uos; ++i)
  {
...
void
FiniteStrainViscoPlastic::computeIntVarRateDerivatives()
{
  Real val = 0;
...
void
FiniteStrainViscoPlastic::computeIntVarDerivatives()
{
  Real val = 0;
...
void
FiniteStrainViscoPlastic::computeStrengthDerivatives()
{
  Real val = 0;
