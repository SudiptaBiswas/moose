...
validParams<ComputeStrainBase>()
{
  ...
  params.addParam<MaterialPropertyName>(
      "periodic_strain", "Material Propertyname for calculating periodic strain.");
...
ComputeStrainBase::ComputeStrainBase(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _periodic_strain(isParamValid("periodic_strain")
                      ? &getMaterialProperty<RankTwoTensor>(_base_name + "periodic_strain")
                      : NULL),
