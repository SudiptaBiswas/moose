...
reader0 = chigger.exodus.ExodusReader('GrandPotentialPlanarGrowth_test2_out.e')
...
time = chigger.annotations.TimeAnnotation(layer=2, font_size=18, color=[1,0,1], prefix='Time:',
                                          justification='center', position=[0.8,0.9],
                                          vertical_justification='middle', notation='fixed', precision=4)

moose = ImageAnnotation(filename='moose.png', scale=0.8, opacity=0.12)
inl = ImageAnnotation(filename='inl.png', position=[0.05, 0.98], scale=0.07,
                      horizontal_alignment='left', vertical_alignment='top')
text1 = chigger.annotations.TextAnnotation(text='Dendrite Growth during Directional Solidification', font_size=20, text_color=[1,1,1],
...
for i in range(0,200):
    ...
    window.write('directional_solidification1_%04d.png' % i)

window.start()

img2mov('directional_solidification1_*.png', 'directional_solidification1.mp4', duration=10)
