...
GrainForcesPostprocessor::execute()
{
  ...
  for (unsigned int i=0; i< _total_grains; ++i)
  {
    ...
    {
      _grain_force_torque_vector[12*i+6] += _grain_force_jacobians[i*total_dofs+j](0);
      _grain_force_torque_vector[12*i+7] += _grain_force_jacobians[i*total_dofs+j](1);
      _grain_force_torque_vector[12*i+8] += _grain_force_jacobians[i*total_dofs+j](2);
      _grain_force_torque_vector[12*i+9] += _grain_torque_jacobians[i*total_dofs+j](0);
      _grain_force_torque_vector[12*i+10] += _grain_torque_jacobians[i*total_dofs+j](1);
      _grain_force_torque_vector[12*i+11] += _grain_torque_jacobians[i*total_dofs+j](2);
    }
  }
}
