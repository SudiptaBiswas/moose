...
InputParameters validParams<ConstantGrainForceAndTorque>()
{
  ...
  // params.addCoupledVariable("c","concentration field");
  // params.addCoupledVariable("etas", "order parameters");
...
ConstantGrainForceAndTorque::execute()
{
  ...
  //   }
    ...
    // for (unsigned int i = 0; i < _ncrys; ++i)
    //
    // RealGradient _dF = _c[_qp] * _grad_vals[i][_qp];
    //
      // _force_torque_store[6*i+0] += _dF(0)*;
      // _force_torque_store[6*i+1] += _dF(1);
      // _force_torque_store[6*i+2] += _dF(2);
      // _force_torque_store[6*i+3] += 0.0;
      // _force_torque_store[6*i+4] += 0.0;
      // _force_torque_store[6*i+5] += 0.0;

      _force_torque_store[0] = _dF(0);
      _force_torque_store[1] = _dF(1);
      _force_torque_store[2] = _dF(2);
      _force_torque_store[3] = 0.0;
      _force_torque_store[4] = 0.0;
      _force_torque_store[5] = 0.0;
      _force_torque_store[6] = - _dF(0);
      _force_torque_store[7] = - _dF(1);
      _force_torque_store[8] = - _dF(2);
      _force_torque_store[9] = 0.0;
      _force_torque_store[10] = 0.0;
      _force_torque_store[11] = 0.0;
