...
OldSingleGrainRigidBodyMotion::OldSingleGrainRigidBodyMotion(const InputParameters & parameters) :
...
Real
OldSingleGrainRigidBodyMotion::computeQpResidual()
{
  return _velocity_advection * _grad_u[_qp] * _test[_i][_qp];
}

Real
OldSingleGrainRigidBodyMotion::computeQpJacobian()
{
  return _velocity_advection * _grad_phi[_j][_qp] * _test[_i][_qp]
  + _velocity_advection_derivative_eta[_op_index] * _phi[_j][_qp] * _grad_u[_qp] * _test[_i][_qp];
        // + _velocity_advection_derivative_eta[_op_index].cross(_grad_phi[_j][_qp]) * _grad_u[_qp] * _test[_i][_qp];
}

Real
OldSingleGrainRigidBodyMotion::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _c_var)
    return _velocity_advection_derivative_c * _grad_u[_qp] * _phi[_j][_qp] * _test[_i][_qp];

  for (unsigned int jvar_index = 0; jvar_index < _op_num; ++jvar_index)
    if (jvar == _vals_var[jvar_index])
      return _velocity_advection_derivative_eta[jvar_index] * _phi[_j][_qp] * _grad_u[_qp] * _test[_i][_qp];
      //return _velocity_advection_derivative_eta[jvar_index].cross(_grad_phi[_j][_qp]) * _grad_u[_qp] * _test[_i][_qp];

  return 0.0;
}

void
OldSingleGrainRigidBodyMotion::calculateAdvectionVelocity()
{
  _velocity_advection = 0.0;
  _velocity_advection_derivative_c = 0.0;
  _velocity_advection_derivative_eta.resize(_op_num);
