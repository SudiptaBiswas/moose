...
SingleGrainRigidBodyMotion::getUserObjectJacobian(unsigned int jvar, dof_id_type dof_index)
{
  ...
  if (grain_id != FeatureFloodCount::invalid_id)
  {
    ...
    if (jvar == _c_var)
    {
      _force_jacobian(0) = _grain_force_c_jacobians[(6*grain_id+0)*_total_dofs+dof_index];
      _force_jacobian(1) = _grain_force_c_jacobians[(6*grain_id+1)*_total_dofs+dof_index];
      _force_jacobian(2) = _grain_force_c_jacobians[(6*grain_id+2)*_total_dofs+dof_index];
      _torque_jacobian(0) = _grain_force_c_jacobians[(6*grain_id+3)*_total_dofs+dof_index];
      _torque_jacobian(1) = _grain_force_c_jacobians[(6*grain_id+4)*_total_dofs+dof_index];
      _torque_jacobian(2) = _grain_force_c_jacobians[(6*grain_id+5)*_total_dofs+dof_index];
    }

    for (unsigned int jvar_index = 0; jvar_index < _op_num; ++jvar_index)
      if (jvar == _vals_var[jvar_index])
      {
        _force_jacobian(0) = _grain_force_eta_jacobians[jvar_index][(6*grain_id+0)*_total_dofs+dof_index];
        _force_jacobian(1) = _grain_force_eta_jacobians[jvar_index][(6*grain_id+1)*_total_dofs+dof_index];
        _force_jacobian(2) = _grain_force_eta_jacobians[jvar_index][(6*grain_id+2)*_total_dofs+dof_index];
        _torque_jacobian(0) = _grain_force_eta_jacobians[jvar_index][(6*grain_id+3)*_total_dofs+dof_index];
        _torque_jacobian(1) = _grain_force_eta_jacobians[jvar_index][(6*grain_id+4)*_total_dofs+dof_index];
        _torque_jacobian(2) = _grain_force_eta_jacobians[jvar_index][(6*grain_id+5)*_total_dofs+dof_index];
      }

    const auto force_jac = _mt / volume * _force_jacobian;
    const auto torque_jac = _mr / volume * _torque_jacobian.cross(_current_elem->centroid() - centroid);
