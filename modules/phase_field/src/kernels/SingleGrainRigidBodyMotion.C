...
{
  const auto grain_id = _grain_ids[_op_index];
  if (grain_id != FeatureFloodCount::invalid_id)
  {
    ...
    RealGradient force_jacobian = 0;
    RealGradient torque_jacobian = 0;

    if ((_var.dofIndices()[0] == 286 || _var.dofIndices()[1] == 286 || _var.dofIndices()[2] == 286 || _var.dofIndices()[3] == 286)
        && dof_index == 332)
      std::cout << "value = " << _grain_force_c_jacobians[332] << std::endl;
