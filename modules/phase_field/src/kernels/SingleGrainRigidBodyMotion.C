...
SingleGrainRigidBodyMotion::computeQpJacobian()
{
  ...
  if (grain_index != libMesh::invalid_uint)
  {
    const auto volume = _grain_tracker.getGrainVolume(grain_index);
    const auto centroid = _grain_tracker.getGrainCentroid(grain_index);
    getUserObjectEtaJacobians(_var.dofIndices()[_j], _op_index, grain_index);
...
SingleGrainRigidBodyMotion::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (grain_index != libMesh::invalid_uint)
  {
    if (jvar == _c_var)
    {
      ...
      getUserObjectCJacobians(jv.dofIndices()[_j], grain_index);
    ...
    for (unsigned int i = 0; i < _op_num; ++i)
      if (jvar == _vals_var[i])
      {
        ...
        getUserObjectEtaJacobians(jv.dofIndices()[_j], i, grain_index);
  ...
  if (grain_index != libMesh::invalid_uint)
  {
    ...
    for (unsigned int i=0; i<_op_num; ++i)
      if (jvar == _vals_var[i])
      {
        const auto volume = _grain_tracker.getGrainVolume(grain_index);
        const auto centroid = _grain_tracker.getGrainCentroid(grain_index);
        getUserObjectEtaJacobians(dof_index, i, grain_index);
