...
SingleGrainRigidBodyMotion::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _c_var)
  {
    ...
    return _velocity_advection_derivative_c[_qp][_op_index][jv.dofIndices()[_j]] * _grad_u[_qp] * _test[_i][_qp]
           + _div_velocity_advection_derivative_c[_qp][_op_index][jv.dofIndices()[_j]] * _u[_qp] * _test[_i][_qp];
  }
...
{
  if (jvar == _c_var)
    return _velocity_advection_derivative_c[_qp][_op_index][dof_index] * _grad_u[_qp] * _test[_i][_qp]
           + _div_velocity_advection_derivative_c[_qp][_op_index][dof_index] * _u[_qp] * _test[_i][_qp];
