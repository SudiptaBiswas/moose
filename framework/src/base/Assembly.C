...
{
  std::vector<dof_id_type> global_indices(_dof_map.n_dofs());
  ...
  for (const auto & ivar : vars)
    for (const auto & jvar : vars)
      if ((*_cm)(ivar->number(), jvar->number()) != 0 && _jacobian_block_nonlocal_used[ivar->number()][jvar->number()])
        addJacobianBlock(jacobian, jacobianBlockNonlocal(ivar->number(), jvar->number()), ivar->dofIndices(), global_indices, ivar->scalingFactor());
}
...
Assembly::cacheJacobianNonlocal()
{
  ...
  for (const auto & ivar : vars)
    for (const auto & jvar : vars)
      if ((*_cm)(ivar->number(), jvar->number()) != 0 && _jacobian_block_nonlocal_used[ivar->number()][jvar->number()])
        cacheJacobianBlockNonlocal(jacobianBlockNonlocal(ivar->number(), jvar->number()), ivar->dofIndices(), global_indices, ivar->scalingFactor());
}
