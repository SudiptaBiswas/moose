    ...
    Kernel(parameters)
{
  ...
  // mooseWarning("NonlocalKernel is an experimental capability which is computationally expensive.");
}
    ...
    if (it == local_dofindices.end()) // eleminating the local components
      for (_i = 0; _i < _test.size(); _i++)
        for (_qp = 0; _qp < _qrule->n_points(); _qp++)
          keg(_i, var_alldofindices[_k]) += _JxW[_qp] * _coord[_qp] * computeQpNonlocalJacobian(var_alldofindices[_k]);
  }
}
